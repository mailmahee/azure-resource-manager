...
# Following file will modify to update scylla version
readonly TEMPLATE_FILE="scylla_deployment.json"
...
# Current Version in scylla_deployment.json
current_version="$(grep -A 2 "ScyllaVersion\":" "${TEMPLATE_FILE}" | grep "defaultValue" | cut -d '"' -f 4)"
...
#  1. scylla_deployment.json"
#    1.1. "defaultValue": "1.1.0"
#  2. README.md
#    2.1. ScyllaVersion='1.1.0'

echo "Updating..."

# Update Version in scylla_deployment.json
sed -i -E "/ScyllaVersion\":/,+2 s/[0-9]+.[0-9]+.[0-9]+.[0-9]+/"${release_version}"/" "${TEMPLATE_FILE}"

# Update Version in README.md
sed -i -E "/ScyllaVersion=/,+1 s/[0-9]+.[0-9]+.[0-9]+.[0-9]+/"${release_version}"/" "${README}"
